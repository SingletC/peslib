...
!-->  one has to call this subroutine one and only one before calling the getpot() subroutine
      ...
      character(len=256) :: bias_file, weights_file
      integer :: status
      
      ! Try different possible paths for the data files
      ! First try: peslib/data/O2H3/
      bias_file = "peslib/data/O2H3/biases.txt"
      weights_file = "peslib/data/O2H3/weights.txt"
      
      open(4, file=bias_file, status='old', iostat=status)
      if (status /= 0) then
        ! Second try: src/O2H3/
        bias_file = "src/O2H3/biases.txt"
        weights_file = "src/O2H3/weights.txt"
        open(4, file=bias_file, status='old', iostat=status)
        if (status /= 0) then
          ! Third try: current directory
          bias_file = "biases.txt"
          weights_file = "weights.txt"
          open(4, file=bias_file, status='old', iostat=status)
          if (status /= 0) then
            write(*,*) 'Error: Cannot find biases.txt file'
            write(*,*) 'Tried paths:'
            write(*,*) '  peslib/data/O2H3/biases.txt'
            write(*,*) '  src/O2H3/biases.txt'
            write(*,*) '  biases.txt'
      ...
      if (status /= 0) then
        write(*,*) 'Error: Cannot find weights.txt file'
