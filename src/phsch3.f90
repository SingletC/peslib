...
!   Common name:                PhSCH3 II (Coupled PESs, Surface set II)
!   Number of derivatives:      1
!   Number of bodies:           16
!   Number of electronic surfaces: 3
!   Interface: Section-2
!
!   Reference: "Full-dimensional ground- and excited-state potential energy surfaces and
! state couplings for photodissociation of thioanisole"
! Shaohong L. Li and Donald G. Truhlar, J. Chem. Phys., 146, 064301 (2017).
!
! "Improved Potential Energy Surfaces of Thioanisole and the Effect of Upper Surface
! Variations on the Product Distribution Upon Photodissociation"
! Yinan Shu and Donald G. Truhlar, Chem. Phys., Submitted Mar. 2018
!
!   Notes:
!   Coupled 42-dimensional PESs for electronically nonadiabatic
!   photodissociation of thioanisole (C7H8S) into thiophenoxyl and methyl radicals.
!   Diabatic potential energy matrix and its gradients, adiabatic potential
!   energies and their gradients, nonadiabatic vector couplings, and the
!   diabatic-to-adiabatic transformation matrix for the three lowest diabatic or
!   adiabatic electronic states are available.
!**********************************************************************
...
!                  Yinan Shu, Mar. 2018
!
! This surface corresponds to the surface II in the 2nd ref. 
!
!
...
!   146, 064301 (2017).
!   "Improved Potential Energy Surfaces of Thioanisole and 
!   the Effect of Upper Surface Variations on 
!   the Product Distribution Upon Photodissociation"
!   by Yinan Shu and Donald G. Truhlar, Chem. Phys.
!   Submitted Mar. 2018
...
subroutine calcu3_ps(uu, guu, x, qps)
  ...
  b       =    2.142 
  A       =   -668.352732063236     
  BB      =   -6.296612817531474d-002
  alpha1  =   0.721710258187793     
  Rw      =    1.63756083461647     
  CC      =   -4.081069027402d-002
  !--- The following are unused after treating theta as tertiary;
  !---   keep them just to avoid messing up other parts
  ...
  !--- (d U3)/(d phi)
  gradu3(2) = - 0.002*(sin(phi))*(cos(phi)) &
              + 2*W1*sin(2*phi)
  !--- (d U3)/(d theta)
  gradu3(3) = 0d0
...
subroutine calcuij_t(uu, guu, x, qps, qtc, nqtc, &
  ...
  subroutine uij_t(uij, duij, x, qps, qtc, nqtc, &
    ...
    !--- The attenuation function is used to damp the coupling to zero 
    !--- The following attenuation function is used
    !       [ 1                       , R<R2
    !  f(R)=[ g(R-R2)                 , R2<= R <= R2+0.2Angstrom
    !       [ exp(-1.66661*(R-R2))    , R >= R+0.2Angstrom
    ! The parameter 1.66661 is obtained by fitting the ground state morse
    ! potential.
    !  g(R)=-2951.3853314209523*(R-R2)^5+1562.7687268060167*(R-R2)^4-229.93090322936524*(R-R2)^3+1
    !  where R2 is the R value of the last anchor point
    !  the current g(R) makes f(R) smooth up to second derivative  
    do iR0 = 1, nR0
       if (iR0<nR0) then
           atten(iR0) = 1d0; datten(iR0) = 0d0
       else if (iR0.eq.nR0) then
           if (R<R0(nR0)) then
               atten(iR0) = 1d0
               datten(iR0) = 0d0
           else if (R.eq.R0(nR0)) then
               atten(iR0) = 1d0
               datten(iR0) = 0d0
           else if (R>R0(nR0)+0.2) then
               atten(iR0) = exp(-2d0*(R-R0(nR0)))
               datten(iR0) = -2d0*exp(-2*(R-R0(nR0)))
           else 
               atten(iR0) = -2951.3853314209523*(R-R0(nR0))**5d0 &
                            +1562.7687268060167*(R-R0(nR0))**4d0 &
                            -229.93090322936524*(R-R0(nR0))**3d0 + 1
               datten(iR0) = -14756.92665710476*(R-R0(nR0))**4d0 &
                             +6251.074907224067*(R-R0(nR0))**3d0 &
                             -689.7927096880958*(R-R0(nR0))**2d0
           end if
       end if       
    end do
    !--- end calculation of attenuation function
    ...
    do iR0 = 1, nR0
       do iphi0 = 1, nphi0
          ...
          !--- (dtentq(0) is duij/dq)
          dtentR(0) = dtentR(0) &
               +ee(iR0,iphi0)*sij(iphi0)*tentphi(iphi0)&
                *(dtentR(iR0)*atten(iR0)&
                  +tentR(iR0)*datten(iR0))
...
!=================================================================
! *def preopt
!    Dummy subroutine for interfacing with ANT.
!=================================================================
subroutine prepot
  implicit none
  return
